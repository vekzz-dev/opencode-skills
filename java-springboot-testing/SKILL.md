...
1. **Resolve the libraryId** - Use `context7_resolve-library-id`:
   - `libraryName`: "junit 5", "assertj", "mockito", or "spring boot test"
   - `query`: what you need (e.g., "mocking beans", "assertions best practices")

2. **Query the docs** - Use `context7_query-docs`:
   - `libraryId`: "/junit/junit5", "/assertj/assertj", or "/spring/spring-boot"
   - `query`: your specific question about testing

**Before writing tests**, consult Context7 to get updated examples of the library you're going to use.
